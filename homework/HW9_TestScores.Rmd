...
- At first glance, the new classroom scores (ranging from 73 to 79) are all above the old average of 72.6. The mean of the new sample is around 75.6, which is about 3 points higher than the previous district average.
- The trend based on the scores in the classroom seem to have a improvement. However, this is just an initial impression to know whether the improvement is statistically significant (and not just due to chance), we need to run a formal test (like a one-sample t-test).
...
- Could other factors besides the program (such as new teachers, smaller class sizes, or test prep) explain the improvement?
...
- Test Score averages based on the classroom
...
**Hypotheses:**
Null: The new reading program does not improve scores (the mean is less than or equal to 72.6).
Alternative: The new reading program improves scores (the mean is greater than 72.6).
...
- The p-value tells us the probability of observing scores this high (or higher) if the true mean were still 72.6. Since it’s extremely small (< 0.05), we reject the null hypothesis.
...
story_stats <- tibble(
  `Previous average` = 72.6,
  `New average` = mean(75.75),
  `Improvement` = mean(3.15) - 72.6,
...
headline --> Maryland’s New Reading Program Produces Statistically Significant Gains

lead --> Third-grade students in Maryland posted higher reading test scores after the launch of a new reading program. Average scores rose from 72.6 to 75.8, an improvement of more than three points. A statistical test confirms the gains are highly unlikely to be due to chance, supporting the superintendent’s claim that the program is making a difference.
...
- Long-term retention (whether gains persist in higher grades).
