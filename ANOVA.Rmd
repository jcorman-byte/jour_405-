...
Private colleges have a higher mean SAT score then public and community colleges. Community colleges seem to have the lowest SAT scores by a wide margin. Public colleges seem to be in the middle in every single category besides  having a student recieve a 1600 tying private colleges with the highest maximum. It is pretty evident that private universities take the highest SAT students while community colleges recieve the lowest scoring kids.
...
ANSWER HERE: We can calculate the critical value for an F statistic by using the qf() function in R
...
1. Is there a significant difference in SAT scores across college types?
  There is a significant difference because the ANOVA Test shows how different the f value is for the college types. One of the college types goes to 16 decimal places while the other college only comes in at 3 deciamls. That is a crazy difference which confirms the significant difference in SAT scores across college types. 
2. Which specific college types differ from each other?
All colleges differ from each other. Private colleges are proven to get the kids with the higher scores while community colleges take the kid with the lowest SAT scores. 
3. How large are these differences?
These differences are pretty large. The difference between private and community colleges are 366.96. Difference between public and community colleges is 292.55. Finally the difference between public and private colleges is -74.40. These are all very large differences which validates are previous points that there is a significant difference in SAT scores across all college types based on the data. 
